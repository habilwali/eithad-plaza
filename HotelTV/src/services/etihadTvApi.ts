...
export async function fetchEtihadPackages(
  ...
  const packages: EtihadPackageRow[] = [];
  for (const row of rawList) {
    ...
    packages.push({id, name: name || `Package ${id}`, price, image});
  }
...
export async function fetchEtihadChannels(
  ...
  const channels: EtihadChannelRow[] = [];
  for (const row of rawList) {
    if (typeof row !== 'object' || row === null) {continue;}
    const r = row as Record<string, unknown>;
    const id = typeof r.id === 'number' ? r.id : Number(r.id);
    ...
    if (!Number.isFinite(id) || id <= 0) {continue;}
    channels.push({id, name: name || `Channel ${id}`, stream_url, logo, status});
  }
